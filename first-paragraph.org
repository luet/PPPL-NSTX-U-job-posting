Each run on the NSTX-U experiment [give a link here if I didn't give it earlier] consists of a plasma blast or shot [Link to youtube video]. 
...
Through a large array of instrumentation each shot generate large amount of data. 
...
The initial project for this position will be to develop a web [web based?] tool that will be used by everyone involved in the experiment to track the lifecycle of the shots: from the tracking of the initial scientific proposal, the logging of the experimental conditions and to the [visualization/recording] of the data analysis of the results.

The NSTX-U experiment is currently out of service for upgrade, but a previous version of the experiment (NSTX) was in operation from [??] to [??]. 
Some software components that were developped for NSTX can be reused with some upgrades, other components will need to be written from scratch.
...
This role will require a lot of cooperation and communication with a large and diverse group of collaborators: researchers, technical staff ... and carefull project management to meet the deadline.
